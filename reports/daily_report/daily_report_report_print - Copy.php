              ...
              <tr>
				...
				$i=0;
				 ...
				 while($FEss=mysqli_fetch_array($Ess))
				 {
				  ...
				  $sc="select sum(Amount) as Amount from account_cash where ReferenceNo='".$FEss['inv_no']."' AND TransactionFrom='s_purchase'";
				 $Esc=mysqli_query($conn,$sc);
				 $FEsc=mysqli_fetch_array($Esc);
				 
				  $sb="select sum(Amount) as Amount from account_bank where ReferenceNo='".$FEss['inv_no']."' AND TransactionFrom='s_purchase'";
				 ...
				 $sco="select sum(Amount) as Amount from account_cash where ReferenceNo='".$FEss['inv_no']."' AND TransactionFrom='sup_outstanding'";
				 $Esco=mysqli_query($conn,$sco);
				 $FEsco=mysqli_fetch_array($Esco);
				 
				  $sbo="select sum(Amount) as Amount from account_bank where ReferenceNo='".$FEss['inv_no']."' AND TransactionFrom='sup_outstanding'";
