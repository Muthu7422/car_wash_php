  ...
  return false; 
	...
	<tr>
        ...
        <?php 
		 	...
		 	$Es="select SUM(Amount) AS SAmount from account_cash where TransactionType='Credit' and franchisee_id='".$_SESSION['BranchId']."'";
			$Eq=mysqli_query($conn,$Es);
			$Eqi=mysqli_fetch_array($Eq);
			
			$Es1="select SUM(Amount) AS SAmount from account_bank where TransactionType='Credit' and franchisee_id='".$_SESSION['BranchId']."'";
			...
			$Edsa="select DISTINCT(cust_name) from cust_outstanding and franchisee_id='".$_SESSION['BranchId']."'";
			...
			while($iu=mysqli_fetch_array($Rfaw))
			{
				...
				$Bvc=mysqli_fetch_array($Tgf);
					...
					$total=$total+$Bvc['total_outstanding'];
		...
		<td><input type="txt" class="form-control" name="NettProfit" id="NettProfit" placeholder="Nett Profit" value="<?php echo $total; ?>" onKeyPress="return tabE(this,event)" readonly></td>
		</tr>
		<?php
		...
		<?php 
		$Eop="select SUM(ExpenseAmount+TaxAmount) As Land from expense_details where LedgerGroup='15' and ExpenseType='3' and franchisee_id='".$_SESSION['BranchId']."'";
		$Trd=mysqli_query($conn,$Eop);
		$Tde=mysqli_fetch_array($Trd);
		
		$Eop1="select SUM(ExpenseAmount+TaxAmount) As Buildings from expense_details where LedgerGroup='15' and ExpenseType='10' and franchisee_id='".$_SESSION['BranchId']."'";
		$Trd1=mysqli_query($conn,$Eop1);
		$Tde1=mysqli_fetch_array($Trd1);
		
		$Eop2="select SUM(ExpenseAmount+TaxAmount) As Equipment from expense_details where LedgerGroup='15' and ExpenseType='11' and franchisee_id='".$_SESSION['BranchId']."'";
		...
		<td><label for="Branch"></label><?php echo $TotalAssets ?></td>
		 	...
		 	$Es="select SUM(Amount) AS SAmount from account_cash where TransactionType='Debit' and franchisee_id='".$_SESSION['BranchId']."'";
			$Eq=mysqli_query($conn,$Es);
			$Eqi=mysqli_fetch_array($Eq);
			
			$Es1="select SUM(Amount) AS PAmount from account_bank where TransactionType='Debit' and franchisee_id='".$_SESSION['BranchId']."'";
			$Eq1=mysqli_query($conn,$Es1);
			$Eqi1=mysqli_fetch_array($Eq1);
			...
			//Wages Total Amount start****
		  	$Ren="select SUM(Amount) AS AccAmount from account_cash where LedgerGroup='36' and franchisee_id='".$_SESSION['BranchId']."'";
			$Wqa=mysqli_query($conn,$Ren);
			$Fcd=mysqli_fetch_array($Wqa);
			
			$Ren1="select SUM(Amount) AS BaAmount from account_bank where LedgerGroup='36' and franchisee_id='".$_SESSION['BranchId']."'";
			$Wqa1=mysqli_query($conn,$Ren1);
			$Fcd1=mysqli_fetch_array($Wqa1);
			
		 	 $Wages=$Fcd['AccAmount']+$Fcd1['BaAmount']; 
			// Wages total amount End ****
			
			//Office Rent Total Amount start****
		  	$Rens="select SUM(Amount) AS AccAmount from account_cash where LedgerGroup='35' and franchisee_id='".$_SESSION['BranchId']."'";
			$Wqas=mysqli_query($conn,$Rens);
			$Fcds=mysqli_fetch_array($Wqas);
			
			$Ren2="select SUM(Amount) AS BaAmount from account_bank where LedgerGroup='35' and franchisee_id='".$_SESSION['BranchId']."'";
			...
			//Deposits Rent Total Amount start****
		  	$Rena="select SUM(Amount) AS AccAmount from account_cash where LedgerGroup='10' and franchisee_id='".$_SESSION['BranchId']."'";
			$Wqaa=mysqli_query($conn,$Rena);
			$Fcda=mysqli_fetch_array($Wqaa);
			
			$p="select SUM(Amount) AS BaAmount from account_bank where LedgerGroup='10' and franchisee_id='".$_SESSION['BranchId']."'";
	   ...
	   <div class="col-sm-6" style="border-right-style: groove;">
		...
		<tr>
		<td> <label for="Branch">Wages Payable</label></td>
		<td> <input type="number" class="form-control" name="DirectIncomes" value="<?php echo $Wages; ?>" id="DirectIncomes" readonly></td>
		</tr>
		
		<tr>
		<td>  <label for="Branch">Office Rent</label></td>
		<td><input type="number" class="form-control" name="ClosingStock" id="ClosingStock" value="<?php echo $Office; ?>"   readonly></td>
		</tr>
		<tr>
		...
		<?php
		$TotalCurrentLiabilities=$AccountPayable+$Deposits+$Office+$Wages;
		?>
		<tr>
