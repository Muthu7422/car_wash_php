...
//   $echeck="select * from a_customer_vehicle_details where vehicle_no='".$_POST['inputs']."' and status='Active'"; 
	//echo $_POST['inputs'];
	$InputArray=explode("-",$_POST['inputs']);
	$InputChitNo=trim($InputArray[1]);
	$echeck="select * from master_vehicle where Id = '$InputChitNo'";
	...
	echo json_encode(array($BrandName,$SegmentName,$TypeName));	   
